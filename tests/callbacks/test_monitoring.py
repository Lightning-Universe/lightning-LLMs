...
@mock.patch("torch.cuda.max_memory_allocated", lambda: random.randint(1024**3, 10 * 1024**3))
...
def _custom_monitoring_callback_train_mock(
    ...
    for i in range(world_size):
        assert f"{gpu_memory_logname}_rank{i}" in logger.log_metrics.call_args[-1]["metrics"]
    ...
    assert f"{time_per_batch_logname}_averaged10" in logger.log_metrics.call_args[-1]["metrics"]
    assert f"{time_per_batch_logname}_averaged100" in logger.log_metrics.call_args[-1]["metrics"]
    for i in range(world_size):
        assert f"{gpu_util_logname}_rank{i}" in logger.log_metrics.call_args[-1]["metrics"]
        assert f"{gpu_memory_logname}_rank{i}" in logger.log_metrics.call_args[-1]["metrics"]
    ...
    for i in range(world_size):
        assert f"{gpu_util_logname}_rank{i}" in logger.log_metrics.call_args[-1]["metrics"]
        assert f"{gpu_memory_logname}_rank{i}" in logger.log_metrics.call_args[-1]["metrics"]
        assert f"{gpu_util_logname}_rank{i}_averaged10" in logger.log_metrics.call_args[-1]["metrics"]
    ...
    for i in range(world_size):
        ...
        assert f"{gpu_util_logname}_rank{i}_averaged100" in logger.log_metrics.call_args[-1]["metrics"]
...
@pytest.mark.parametrize("time_per_batch_logname", ["time/seconds_per_iter", "foo/bar_time"])
def test_monitoring_callback_train(world_size, gpu_util_logname, gpu_memory_logname, time_per_batch_logname):
